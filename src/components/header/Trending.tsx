    ...
    <div className="relative mt-4 w-full max-w-screen-xl mx-auto">
      ...
      >
        {trendingProducts.map((products, index) => (
          ...
          >
            <div
              className="flex bg-white  rounded-2xl shadow-lg p-6 max-w-3xl flex-col md:flex-row items-center gap-6"
              onClick={() => handleTrendingProducts(products.id)}
            >
              <img
                src={products.image}
                alt={products.title || ` ${index + 1}`}
                className="w-full md:w-1/2 h-64  cursor-pointer object-contain rounded-xl"
              />
